...
#[unsafe(no_mangle)]
#[doc(hidden)]
pub extern "C" fn ts3plugin_configure(
	_handle: *mut std::os::raw::c_void, _qParentWidget: *mut std::os::raw::c_void,
) {
	// !TODO
...
pub unsafe extern "C" fn ts3plugin_onUpdateChannelEvent(server_id: u64, channel_id: u64) {
	...
	if let Err(error) = {
		...
		old_channel = server
			...
			.unwrap_or(crate::ChannelData::new(server_id, channel_id));
...
pub unsafe extern "C" fn ts3plugin_onChannelMoveEvent(
	...
	unsafe {
		...
		if let Some(channel) =
			api.get_mut_server(server_id).and_then(|s| s.get_mut_channel(channel_id))
		{
